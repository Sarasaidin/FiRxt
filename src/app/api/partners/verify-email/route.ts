...
export async function POST(request: Request) {
  try {
    ...
    if (!resendApiKey) {
      console.warn("RESEND_API_KEY is missing. Verification email was not sent.");
      console.log(`Partner verification code for ${email}: ${code}`);

      return NextResponse.json({
        message:
          "Verification code generated. Email sending is not configured locally.",
      });
    }
    ...
    const { error } = await resend.emails.send({
      from: fromEmail,
      to: email,
      subject: "Your FiRxt partner verification code",
      html: `
        <div style="font-family: Arial, sans-serif; color: #1f2a44; line-height: 1.6;">
          <h1 style="margin: 0 0 16px; font-size: 24px;">Verify your partner email</h1>
          <p>Use the verification code below to continue your FiRxt partner registration.</p>
